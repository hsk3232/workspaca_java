...
class LinkedList2 {
	...
	// 전달된 element를 찾을 때 comparator 객체를 사용한다
	{
		...
		while (curr != null) {
			if (cc.compare(curr.data, element) == 0) {
				// 삭제
				if (prev == null) {
					first = curr.link;
				} else {
	...
	// 임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		...
		// 중간/꼬리 삽입
		Node2 prev = first;
		Node2 curr = first.link;
		while (curr != null && cc.compare(element, curr.data) > 0) {
			prev = curr;
			curr = curr.link;
		}
		newNode.link = curr;
	...
	public boolean Search(SimpleObject5 element, Comparator<SimpleObject5> cc) {
		...
		while (curr != null) {
			...
			// 오름차순이므로 현재 노드가 이미 더 크면 이후엔 없음(조기 종료 가능)
	...
	void Merge(LinkedList2 b, Comparator<SimpleObject5> cc) {
		 ...
		 */
		Node2 aPtr = this.first;
		Node2 bPtr = b.first;

		// 어떤 리스트도 비어 있으면 간단 처리
		if (aPtr == null) { this.first = bPtr; b.first = null; return; }
		if (bPtr == null) { return; }

		// 새 머리 결정
		Node2 head, tail;
		if (cc.compare(aPtr.data, bPtr.data) <= 0) {
			head = aPtr;
			aPtr = aPtr.link;
		} else {
			head = bPtr;
		...
		// 남은 꼬리 잇기
		if (aPtr != null) tail.link = aPtr;
		else if (bPtr != null) tail.link = bPtr;

		// 결과를 this.first로, b는 비우기
		this.first = head;
		b.first = null;
...
public class 과제12_8장_리스트_객체연결리스트 {
